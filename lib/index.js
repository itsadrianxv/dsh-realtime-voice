...
import { VOICE_DIRECT_BOOTSTRAP, VOICE_DIRECT_ROUTE, VOICE_DIRECT_STATUS_ROUTE, isDirectDshFunctionName, isDirectFunctionArguments, isDirectVoiceClientControl } from "./direct-protocol.js";
...
function buildDirectMediaOfferBootstrap(config, instructions) {
	return {
...
var DirectControlConnection = class {
	...
	async issueOffer(instructions) {
		...
		const pending = (async () => {
			...
			const effectiveInstructions = instructions ?? buildVoiceInstructions(await new DshVoiceSession(this.ctx, this.hello.target.sessionId).snapshot(), this.continuity);
			const offer = {
				offerId: randomUUID(),
				transport: "websocket",
