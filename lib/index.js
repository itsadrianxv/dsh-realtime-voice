...
var DshVoiceCoordinator = class {
	...
	observeQueue(items) {
		for (const item of items) {
			...
			const promptRpcId = messageSourceRpcId$1(message);
...
function messageSourceRpcId$1(message) {
...
var VoiceRuntime = class {
	...
	acquireLease(request) {
		...
		const mayResume = active !== void 0 && !active.connected && request.resumeId === active.voiceSessionId && request.sessionId === active.sessionId && request.platform === active.platform;
		...
		if (request.resumeId !== void 0 && (resumed === void 0 || resumed.sessionId !== request.sessionId || resumed.platform !== request.platform)) return {
			ok: false,
...
var VoiceConnection = class {
	...
	onProviderEvent(event) {
		...
		switch (event.type) {
			...
			case "response.audio.delta": {
				...
				if (effectiveResponseId !== void 0) this.responseStreams.set(effectiveResponseId, this.outputStreamId);
			...
			case "response.done": {
				...
				const suppressed = responseId !== void 0 && this.suppressedResponses.has(responseId);
				const streamId = responseId === void 0 ? void 0 : this.responseStreams.get(responseId);
				if (!suppressed && streamId !== void 0) {
					...
					if (streamId === this.gatedOutputStreamId && responseId !== void 0) this.schedulePlaybackFallback(streamId, this.responseAudioDurationMs.get(responseId) ?? 0, this.responseAudioStartedAt.get(responseId) ?? Date.now());
				}
	...
	*/
	...
	shouldGateInputDuringPlayback() {
		return this.hello !== void 0 && this.hello.client.duplex !== "full";
	}
	/**
...
function isTransientDisconnect(reason) {
	return reason === "client-disconnected" || reason === "client-error" || reason === "provider-disconnected" || reason === "browser-audio-send-failed";
}
