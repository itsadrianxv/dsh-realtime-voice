...
const SessionId$2 = (value) => value;
const RpcId$3 = (value) => value;
...
var DshVoiceCoordinator = class {
	...
	async handoff(request, spokenInput) {
		...
		const response = await this.ctx.apiProxy.sessions.prompt({
			rpcId: promptRpcId,
			payload: {
				sessionId: SessionId$2(this.sessionId),
				mode,
				content: [{
					type: "text",
	...
	async cancel(reason = "") {
		...
		const response = await this.ctx.apiProxy.sessions.cancel({
			rpcId: this.rpcId(),
			payload: { sessionId: SessionId$2(this.sessionId) }
		});
	...
	async resolveApproval(approvalId, outcome) {
		...
		const receipt = await this.ctx.apiProxy.respond({
			...
			rpcId: RpcId$3(pending.rpcId),
			result: {
				ok: true,
				value: {
					sessionId: SessionId$2(pending.sessionId),
...
const SessionId$1 = (value) => value;
const RpcId$2 = (value) => value;
...
var DshVoiceSession = class {
	...
	async lastAssistantText() {
		const response = await this.ctx.apiProxy.sessions.history({
			rpcId: this.rpcId(),
			payload: {
				sessionId: SessionId$1(this.sessionId),
				maxMessages: 12
			}
		});
		if (!response.result.ok) throw new Error(response.result.error.message);
...
function apply(ctx, config) {
	ctx.inject(["sessionController"], (sessionCtx) => {
		...
		ctx.provide("apiProxy", {
			sessions: {
				list: async ({ payload }) => ok(await controller.list(payload)),
				prompt: async ({ payload }) => ok(await controller.prompt(payload, new AbortController().signal)),
				updateQueue: async ({ payload }) => ok(controller.updateQueue(payload)),
				cancel: async ({ payload }) => ok(controller.cancel(payload)),
				history: async ({ payload }) => ok({ events: (await controller.page({
					sessionId: payload.sessionId,
					limit: payload.maxMessages
				}, new AbortController().signal)).events })
			},
			events: {
				host: (_request, signal) => createLegacyEventStream(sessionCtx, "host", signal),
				mux: (_request, signal) => createLegacyEventStream(sessionCtx, "mux", signal, responders)
			},
			respond: async ({ rpcId, result }) => {
				const resolve = responders.get(String(rpcId));
				if (resolve === void 0) return {
					accepted: false,
					reason: "response is no longer pending"
				};
	...
	ctx.inject(["settings"], (settingsCtx) => {
		const scope = settingsCtx.settings.register(REALTIME_VOICE_SETTINGS_NAMESPACE, Config, { base: config });
...
function createLegacyEventStream(ctx, kind, signal, responders = /* @__PURE__ */ new Map()) {
	const queue = [];
	let wake;
	let closed = false;
	const push = (payload) => {
	...
	const disposers = kind === "host" ? [ctx.on("api-session/status", (sessionId, running) => push({
		type: "host/session-status",
		sessionId,
		running
	})), ctx.on("agent/error", (value) => {
		const event = value;
		...
		ctx.on("session/event", (session, event) => {
			...
			push({
				type: "session/event",
				sessionId: value?.id ?? value?.sessionId,
				event
			});
		}),
		ctx.on("approval/request", (request) => {
			...
			push({
				...
				sessionId: request.agent?.session?.id ?? request.agent?.id,
				...request,
				rpcId
			});
