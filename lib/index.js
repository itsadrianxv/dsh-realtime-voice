...
var DashScopeRealtime = class {
		...
		await new Promise((resolve, reject) => {
			...
			const fail = (error) => {
				...
				if (!settled) {
					settled = true;
					reject(error);
				} else if (!this.closed) this.callbacks.onEvent({
					type: "error",
					error: {
	...
	handleEvent(event) {
		...
		if (event.type === "response.function_call_arguments.done") {
			...
			pending.push({
				call,
				result: this.callbacks.onTool(call)
			});
		...
		if (responseId === void 0) return;
		const pending = this.pendingTools.get(responseId);
		if (pending === void 0 || pending.length === 0) return;
...
var DshVoiceTools = class {
	...
	async status() {
		const list = await this.ctx.apiProxy.sessions.list({
			rpcId: this.rpcId(),
			payload: {}
		});
