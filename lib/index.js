...
var VoiceRuntime = class {
	...
	acquire(resumeId, sessionId) {
		this.sweep();
		const resumed = resumeId === void 0 ? void 0 : this.calls.get(resumeId);
		if (resumed !== void 0 && resumed.sessionId === sessionId) {
	...
	sweep() {
		...
		for (const [id, state] of this.calls) if (state.lastSeenAt < expiredBefore) this.calls.delete(id);
	}
};
//#endregion
