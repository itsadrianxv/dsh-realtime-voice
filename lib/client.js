		...
		var VoiceCallController = class {
			...
			async connect(sessionId) {
				...
				await new Promise((resolve, reject) => {
					...
					socket.onerror = () => reject(/* @__PURE__ */ new Error("无法连接 DSH 实时语音插件。"));
					const ready = (event) => {
			...
			receive(event) {
				...
				switch (message.type) {
					...
					case "voice.agent-status":
						...
						return;
					case "voice.error":
						if (message.recoverable) this.update({
							...this.snapshot,
			...
			scheduleReconnect(sessionId) {
				...
				if (this.reconnectAttempt >= 4) {
					this.fail("实时语音连接多次重试失败，DSH 中已经开始的任务不会被取消。");
					return;
				}
				const delay = 1e3 * 2 ** this.reconnectAttempt;
