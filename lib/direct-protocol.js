...
const VOICE_DIRECT_TRANSCRIPT = "dsh.voice.transcript.v1";
const DIRECT_FUNCTION_ARGUMENT_MAX_BYTES = 16384;
const DIRECT_TRANSCRIPT_MAX_ITEMS = 16;
const DIRECT_TRANSCRIPT_MAX_TEXT_CHARS = 4e3;
const DIRECT_TRANSCRIPT_MAX_BYTES = 16384;
...
function isDirectHello(value) {
	...
	return value.protocol === "dsh.voice.direct.v1" && (value.intent === void 0 || value.intent === "connect" || value.intent === "release") && isShortString(value.requestId, 128) && isRecord(client) && isPlatform(client.platform) && isShortString(client.version, 64) && typeof client.foregroundOnly === "boolean" && typeof client.websocketAuthorizationHeader === "boolean" && isRecord(target) && isShortString(target.sessionId, 256) && (resume === void 0 || isRecord(resume) && isShortString(resume.voiceSessionId, 256) && isNonNegativeInteger(resume.lastServerSeq) && isNonNegativeInteger(resume.lastBackendEventSeq) && (resume.transcriptCheckpoint === void 0 || isDirectTranscriptCheckpoint(resume.transcriptCheckpoint))) && (value.intent !== "release" || resume !== void 0 && isRecord(resume) && resume.transcriptCheckpoint === void 0);
}
function isDirectTranscriptCheckpoint(value) {
	if (!isRecord(value) || value.version !== "dsh.voice.transcript.v1" || !Array.isArray(value.items)) return false;
	if (Object.keys(value).some((key) => key !== "version" && key !== "items")) return false;
	if (value.items.length > 16) return false;
	let totalBytes = 0;
	for (const item of value.items) {
		if (!isRecord(item) || Object.keys(item).some((key) => key !== "role" && key !== "text" && key !== "final")) return false;
		if (item.role !== "user" && item.role !== "assistant" || item.final !== true || typeof item.text !== "string" || item.text.trim() === "" || item.text.length > 4e3) return false;
		totalBytes += new TextEncoder().encode(item.text).byteLength;
		if (totalBytes > 16384) return false;
	}
...
//#endregion
export { DIRECT_DSH_FUNCTION_NAMES, DIRECT_FUNCTION_ARGUMENT_MAX_BYTES, DIRECT_TRANSCRIPT_MAX_BYTES, DIRECT_TRANSCRIPT_MAX_ITEMS, DIRECT_TRANSCRIPT_MAX_TEXT_CHARS, VOICE_DIRECT_BOOTSTRAP, VOICE_DIRECT_PROTOCOL, VOICE_DIRECT_ROUTE, VOICE_DIRECT_STATUS_ROUTE, VOICE_DIRECT_TRANSCRIPT, isDirectDshFunctionName, isDirectFunctionArguments, isDirectTranscriptCheckpoint, isDirectVoiceClientControl };
